...
function App() {
  ...
  return (
    ...
    <div className="contaienr">
      ...
      <button className='ms-3 btn btn-secondary' onClick={handleAddTodo}>Add Todo</button>
     <div className='container-fluid'>
        <div style={{backgroundColor:'#4444',height:"65vh"}} className='row mt-5 p-5'>
          <ul>
            {todos.map(todo => (
              <li
                key={todo.id}
              ...
              >
                {todo.text}
                <button className='float-end btn btn-danger' onClick={() => handleDeleteTodo(todo.id)}>Delete</button>
              </li>
            ))}
          </ul>
        </div>
